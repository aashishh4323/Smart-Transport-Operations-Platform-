...
export default function FleetPage() {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      if (selectedVehicle) {
        await updateVehicle({ ...selectedVehicle, ...formState });
      } else {
    ...
    <div className="space-y-6">
      <PageHeader title="Fleet" description="Manage the vehicle registry and availability states." action={
        <Dialog open={isDialogOpen} onOpenChange={(open) => {
          ...
          <DialogTrigger render={<Button onClick={() => setIsDialogOpen(true)}><Plus className="mr-2 h-4 w-4" /> Add vehicle</Button>} />
