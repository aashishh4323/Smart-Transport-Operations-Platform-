...
export default function DriversPage() {
  ...
  const resetForm = () => {
    ...
    setErrorMsg("");
    setFormState({ name: "", licenseNumber: "", licenseCategory: "Class A", licenseExpiry: "2028-01-01", contact: "", safetyScore: 85, status: "Available", region: "Mumbai" });
  };
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      if (selectedDriver) {
        await updateDriver({ ...selectedDriver, ...formState });
      } else {
    ...
    <div className="space-y-6">
      <PageHeader title="Drivers" description="Track license validity and dispatch readiness." action={
        <Dialog open={isDialogOpen} onOpenChange={(open) => {
          ...
          <DialogTrigger render={<Button onClick={() => setIsDialogOpen(true)}><Plus className="mr-2 h-4 w-4" /> Add driver</Button>} />
