...
export default function TripsPage() {
  const { trips, vehicles, drivers, addTrip, dispatchTrip, completeTrip, cancelTrip } = useAppState();
    ...
    <div className="space-y-6">
      <PageHeader title="Trips" description="Dispatch, monitor, complete, and cancel trips from one place." action={
        <Dialog onOpenChange={(open) => (!open ? resetForm() : undefined)}>
          <DialogTrigger><Button><Plus className="mr-2 h-4 w-4" /> New trip</Button></DialogTrigger>
          <DialogContent className="sm:max-w-xl">
            <DialogHeader><DialogTitle>Create draft trip</DialogTitle><DialogDescription>Draft trips can be dispatched once the selected vehicle and driver are valid.</DialogDescription></DialogHeader>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="grid gap-4 md:grid-cols-2"><div className="space-y-2"><Label>Vehicle</Label><Select value={formState.vehicleId} onValueChange={(value) => setFormState({ ...formState, vehicleId: value ?? "" })}><SelectTrigger><SelectValue placeholder="Select vehicle" /></SelectTrigger><SelectContent>{availableVehicles.map((vehicle) => <SelectItem key={vehicle.id} value={vehicle.id}>{vehicle.registrationNumber} · {vehicle.model}</SelectItem>)}</SelectContent></Select></div><div className="space-y-2"><Label>Driver</Label><Select value={formState.driverId} onValueChange={(value) => setFormState({ ...formState, driverId: value ?? "" })}><SelectTrigger><SelectValue placeholder="Select driver" /></SelectTrigger><SelectContent>{availableDrivers.map((driver) => <SelectItem key={driver.id} value={driver.id}>{driver.name} · {driver.licenseCategory}</SelectItem>)}</SelectContent></Select></div></div>
      ...
      <div className="grid gap-4 xl:grid-cols-2">{filteredTrips.map((trip) => {
        ...
        return <div key={trip.id} className="rounded-[24px] border border-[rgba(20,22,26,0.08)] bg-[rgba(255,255,255,0.8)] p-4 shadow-[0_14px_40px_rgba(15,18,25,0.05)] backdrop-blur-[16px]">
          ...
          <div className="mt-4 flex flex-wrap gap-2">{trip.status === "Draft" ? <><Button size="sm" onClick={() => dispatchTrip(trip.id)}>Dispatch</Button><Button size="sm" variant="outline" onClick={() => setSelectedTrip(trip)}>Edit</Button></> : null}{trip.status === "Dispatched" ? <><Button size="sm" onClick={() => completeTrip(trip.id, (vehicle?.odometerKm ?? 0) + trip.plannedDistanceKm, 40)}>Complete</Button><Button size="sm" variant="outline" onClick={() => cancelTrip(trip.id)}>Cancel</Button></> : null}</div>
        </div>;
      })}</div>
    </div>
  );
