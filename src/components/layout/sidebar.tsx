    ...
    <aside className="hidden min-h-screen w-[240px] flex-col border-r border-[rgba(20,22,26,0.08)] bg-[rgba(20,21,24,0.94)] text-white lg:flex">
      ...
      <div className="border-t border-white/10 p-4">
        <div className="rounded-2xl bg-white/8 p-3">
          <p className="text-[13px] font-medium text-white">Signed in as {user?.name || "User"}</p>
          <p className="mt-1 text-[12px] text-white/55">{user?.role || "Role"}</p>
        </div>
        <button 
          onClick={signOut}
          className="mt-3 flex w-full items-center justify-center rounded-2xl border border-white/10 px-3 py-2 text-[13px] text-white/80 transition hover:bg-white/8"
        >
          Sign out
        </button>
      </div>
